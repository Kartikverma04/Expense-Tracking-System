...
def add_update_tab():
    selected_date = st.date_input("Enter Date", value=date.today(), label_visibility="collapsed")
    ...
    with st.form(key=f"expense_form_{selected_date}"):
        ...
        for i in range(5):
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                amount_input = st.number_input(label="Amount", min_value=0.0, step=1.0, value=amount, key=f"amount_{i}_{selected_date}", 
